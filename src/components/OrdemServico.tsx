...
interface AtendimentoOS {
  ...
  hora_chave: number | null
  hora_motor: number | null
  hora_eletrica: number | null
  created_at: string
  clientes: { razao_social: string; cnpj_cpf: string | null } | null
...
export default function OrdemServico({ atendimentoId, onBack }: Props) {
  const [atendimento, setAtendimento] = useState<AtendimentoOS | null>(null)
  ...
  async function carregar() {
    const { data: atend } = await supabase
      ...
      .select(`
        ...
        responsavel_telefone, relato_entrada, garantia_status, hora_chave, hora_motor,
        hora_eletrica, created_at,
        clientes ( razao_social, cnpj_cpf ),
    ...
    if (atend) setAtendimento(atend as unknown as AtendimentoOS)

    const { data: pecasData } = await supabase
      .from('atendimento_pecas')
      .select('id, codigo, descricao, quantidade, unidade, observacao')
    ...
    const { data: servicosData } = await supabase
      ...
      .select('id, codigo, descricao, tempo_padrao, quantidade, unidade, observacao')
      .eq('atendimento_id', atendimentoId)
      .order('descricao')
    ...
    const { data: valData } = await supabase
      ...
      .select('acao, observacao, created_at')
      .eq('atendimento_id', atendimentoId)
      .order('created_at')

    if (valData) setValidacoes(valData)
  ...
  const tempoTotal = servicos.reduce((acc, s) => acc + (s.tempo_padrao || 0) * s.quantidade, 0)
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <header className="bg-blue-900 text-white px-4 py-4 flex items-center gap-4">
        ...
        <h1 className="text-lg font-bold">Ordem de Serviço #{atendimento?.numero}</h1>
      </header>

      <main className="max-w-3xl mx-auto px-4 py-6 space-y-6">
        <div className="bg-white rounded-xl shadow-sm p-5">
          <h2 className="font-bold text-gray-800 mb-3">Identificação</h2>
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div><p className="text-gray-400">Cliente</p><p className="text-gray-800">{atendimento?.clientes?.razao_social || '—'}</p></div>
            <div><p className="text-gray-400">CNPJ/CPF</p><p className="text-gray-800">{atendimento?.clientes?.cnpj_cpf || '—'}</p></div>
            <div><p className="text-gray-400">Responsável</p><p className="text-gray-800">{atendimento?.responsavel_nome || '—'}</p></div>
            <div><p className="text-gray-400">Telefone</p><p className="text-gray-800">{atendimento?.responsavel_telefone || '—'}</p></div>
            ...
            <div><p className="text-gray-400">Hora-chave</p><p className="text-gray-800">{atendimento?.hora_chave || '—'}</p></div>
            <div><p className="text-gray-400">Hora-motor</p><p className="text-gray-800">{atendimento?.hora_motor || '—'}</p></div>
            <div><p className="text-gray-400">Hora-elétrica</p><p className="text-gray-800">{atendimento?.hora_eletrica || '—'}</p></div>
        ...
        {pecas.length > 0 && (
          <div className="bg-white rounded-xl shadow-sm p-5">
            <h2 className="font-bold text-gray-800 mb-3">Peças e Materiais</h2>
              ...
              <tbody>
                {pecas.map((p) => (
                  <tr key={p.id} className="border-b border-gray-50">
                    <td className="py-2 text-gray-800">{p.codigo || '—'}</td>
        ...
        {servicos.length > 0 && (
          <div className="bg-white rounded-xl shadow-sm p-5">
            <h2 className="font-bold text-gray-800 mb-3">Serviços</h2>
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="text-left py-2 text-gray-400 font-medium">Código</th>
              ...
              <tbody>
                {servicos.map((s) => (
                  <tr key={s.id} className="border-b border-gray-50">
                    <td className="py-2 text-gray-800">{s.codigo || '—'}</td>
                    <td className="py-2 text-gray-800">{s.descricao}</td>
                    <td className="py-2 text-right text-gray-800">{s.quantidade}</td>
                    <td className="py-2 text-right text-gray-800">{s.tempo_padrao !== null ? `${s.tempo_padrao} h` : 'Pendente'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {validacoes.length > 0 && (
          <div className="bg-white rounded-xl shadow-sm p-5">
            <h2 className="font-bold text-gray-800 mb-3">Aprovações</h2>
            <div className="space-y-2">
              {validacoes.map((v, i) => (
                <div key={i} className="flex items-center gap-3 text-sm">
                  <span className={`px-2 py-1 rounded text-xs font-medium ${v.acao === 'APROVADO' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{v.acao}</span>
                  <span className="text-gray-500 text-xs">{new Date(v.created_at).toLocaleString('pt-BR')}</span>
                  {v.observacao && <span className="text-gray-600 text-xs">— {v.observacao}</span>}
                </div>
              ))}
            </div>
          </div>
